...
1. 在设备 A 打开设置，点击 `数据导出`，保存导出文件（`.json`）。
2. 在设备 B 打开设置，点击 `数据导入` 并选择该文件。
3. 导入时选择模式：
   - `覆盖现有数据`：用导入文件替换本机统计数据。
   - `合并并累加`：把导入文件与本机数据按日期合并并累加。
4. 若想把两台设备的数据汇总到 B：A 导出后，B 导入时选择 `合并并累加`。
5. 之后在 B 再次导出，回到 A 导入时选择 `覆盖现有数据`，即可让 A 与 B 保持一致。
